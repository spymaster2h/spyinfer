...
// 用于比较浮点数的辅助函数
bool compare_floats(float a, float b, float tolerance = 1e-2f) { return std::abs(a - b) <= tolerance; }


TEST(test_cuda, test_cuda_nrom_fp16)
{
    ...
    std::uniform_real_distribution<float> weight_dis(-1.f, 1.f);
    ...
    for (int n = 0; n < nums; ++n)
    {
        for (int i = 0; i < dims; ++i)
        {
            ...
            float gpu_val = half_to_float(h_gpu_output[n * dims + i]);
             ...
             EXPECT_NEAR(gpu_val, cpu_val, 1e-2)
            << "Mismatch at index " << i << ": expected " << cpu_val
            << ", got " << gpu_val;
        }
    }
