...
{
    if (params.contains("max_num_batched_tokens"))
        config_.max_num_batched_tokens = std::get<int>(params.at("max_num_batched_tokens"));
    if (params.contains("max_num_seqs"))
        config_.max_num_seqs = std::get<int>(params.at("max_num_seqs"));
    if (params.contains("max_model_len"))
        config_.max_model_len = std::get<int>(params.at("max_model_len"));
    if (params.contains("kvcache_block_size"))
        config_.kvcache_block_size = std::get<int>(params.at("kvcache_block_size"));
    if (params.contains("num_kvcache_blocks"))
        config_.num_kvcache_blocks = std::get<int>(params.at("num_kvcache_blocks"));
...
void LLMEngine::append_to_request(int request_id, const std::pair<std::string, std::string>& message) {
    ...
    // Remove BOS/EOS if the standalone encode adds them
    ...
    // Reset status and add back to scheduler
    seq.status = SequenceStatus::WAITING;
    seq.is_prefill = true; // New tokens require a prefill pass
...
void LLMEngine::step() {
    ...
    // Post-step: only remove finished sequence POINTERS from the running queue.
    // The actual Sequence object remains in `all_sequences_`.
...
bool LLMEngine::is_request_finished(int request_id) const {
    ...
    return true; // If not found, it's not running, so it's "finished" in a way.
}

bool LLMEngine::has_unfinished_requests() const {
    return !scheduler_->get_waiting_queue().empty() || !scheduler_->get_running_queue().empty();
}

std::string LLMEngine::get_output(int request_id) const {
    auto it = all_sequences_.find(request_id);
    if (it != all_sequences_.end()) {
        // We need to find the response part, not the whole history
        const auto& tokens = it->second.get_tokens();
        // A simple way: find the last "assistant" prompt and decode from there.
        // This is not perfect. A better way is to track token counts.
        ...
        // If not found, decode everything (for single-shot prompts)
